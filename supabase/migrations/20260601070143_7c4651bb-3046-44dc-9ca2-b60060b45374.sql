...
CREATE OR REPLACE FUNCTION public.denounce_comrade(p_recipient text, p_amount numeric, p_reason text DEFAULT NULL)
...
BEGIN
  ...
  IF v_amt > 5 THEN RAISE EXCEPTION 'Denounce capped at 5 Aura'; END IF;
...
REVOKE EXECUTE ON FUNCTION public.denounce_comrade(text, numeric, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.denounce_comrade(text, numeric, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.denounce_comrade(text, numeric, text) TO service_role;
