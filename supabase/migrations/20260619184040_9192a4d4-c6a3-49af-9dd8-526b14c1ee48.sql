...
BEGIN
  ...
  -- Extended ladder past ×2
  IF    v_total >= 600 * v_n THEN v_mult := 5.0;
  ELSIF v_total >= 540 * v_n THEN v_mult := 4.0;
  ELSIF v_total >= 480 * v_n THEN v_mult := 3.5;
  ELSIF v_total >= 420 * v_n THEN v_mult := 3.0;
  ELSIF v_total >= 360 * v_n THEN v_mult := 2.5;
  ELSIF v_total >= 300 * v_n THEN v_mult := 2.0;
  ELSIF v_total >= 240 * v_n THEN v_mult := 1.5;
  ELSIF v_total >= 180 * v_n THEN v_mult := 1.0;
  ELSE                            v_mult := 0.5;
  END IF;
