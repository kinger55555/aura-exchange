-- #3: send_aura uses sender's rank multiplier instead of fixed 1.5
-- #2: burn_aura lets a comrade truncate their balance to a chosen value; dust → State Bank → Owner when ≥1
...
-- burn_aura: keep exactly p_keep Aura, burn the rest into the State Bank.
-- Owner sweep: when bank ≥1, floor(bank) sent to Owner.
...
REVOKE EXECUTE ON FUNCTION public.burn_aura(numeric) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.burn_aura(numeric) TO authenticated;
GRANT EXECUTE ON FUNCTION public.burn_aura(numeric) TO service_role;